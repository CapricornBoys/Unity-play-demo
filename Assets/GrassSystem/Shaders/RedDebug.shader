Shader "Custom/RedDebug"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // PROPERTY TEST: verify _TrailWorldSize is readable by shader
            half4 frag(G I) : SV_Target
            {
                // If _TrailWorldSize.x > 1.0, the material property is working -> BLUE
                // If _TrailWorldSize.x = 0 (default), property not readable -> RED
                if (_TrailWorldSize.x > 1.0)
                    return half4(0.0, 0.0, 1.0, 1.0); // BLUE = property works
                else
                    return half4(1.0, 0.0, 0.0, 1.0); // RED = property NOT read
            }
            ENDHLSL
        }
    }
